                ...
                <div className="p-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="group">
                                <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 group-focus-within:text-emerald-600 transition-colors ml-1">Project Name <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    required
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all placeholder-gray-400 font-bold text-base"
                                    placeholder="E.g. Website Overhaul"
                                />
                            </div>

                            <div className="bg-gray-50/50 rounded-2xl p-3 border border-gray-100 shadow-sm flex flex-col justify-center">
                                <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-2 ml-1">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {colors.map(color => (
                                        <button
                                            ...
                                            className={`w-6 h-6 rounded-lg transition-all hover:scale-110 shadow-sm border ${formData.color === color.value ? 'ring-2 ring-emerald-500/20 scale-110 border-white shadow-md' : 'border-transparent'}`}
                        ...
                        <div className="group">
                            <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 group-focus-within:text-emerald-600 transition-colors ml-1">Description</label>
                            <textarea
                                rows="2"
                                value={formData.description}
                                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all placeholder-gray-400 font-medium resize-none shadow-sm text-sm"
                                placeholder="Describe the purpose..."
                            ></textarea>
                        </div>

                        <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 space-y-3 shadow-sm">
                            <div className="flex items-center justify-between mb-1">
                                <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                    <i className="fas fa-users"></i> Team Members
                                </h3>
                                <div className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-[9px] font-black uppercase tracking-wider">
                                    {formData.member_ids.length} Selected
                                </div>
                            </div>

                            <div className="relative mb-2 group/search">
                                <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within/search:text-emerald-500 transition-colors text-xs"></i>
                                <input
                                    type="text"
                                    placeholder="Search collaborators..."
                                    className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all text-xs bg-white"
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {collaborators
                                        ...
                                        .map(collab => (
                                            <label
                                                key={collab.id}
                                                className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all border ${formData.member_ids.includes(collab.id)
                        ...
                        <div className="pt-4 flex gap-4 border-t border-gray-100">
                            <button
                                type="button"
                                onClick={() => navigate(-1)}
                                className="flex-1 px-6 py-3 border border-gray-200 text-gray-400 font-bold rounded-xl text-center hover:bg-gray-50 hover:text-gray-600 transition-all uppercase text-[9px] tracking-[0.2em]"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                className="flex-1 px-6 py-3 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-700 hover:to-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-200 hover:shadow-emerald-300 transform transition-all hover:-translate-y-0.5 uppercase text-[9px] tracking-[0.2em]"
                            >
