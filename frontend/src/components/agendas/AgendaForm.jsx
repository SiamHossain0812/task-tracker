...
const AgendaForm = () => {
    ...
    const getSuggestedFinishDate = (category, startDate) => {
        ...
        let daysToAdd = 0;
        switch (category) {
            case 'short': daysToAdd = 2; break;
            case 'mid': daysToAdd = 7; break;
            case 'long': daysToAdd = 15; break;
            default: return '';
        }
    ...
    const [quickAddData, setQuickAddData] = useState({ name: '', email: '', whatsapp_number: '' });
            ...
            try {
                ...
                if (isEdit) {
                    ...
                    setFormData({
                        ...
                        collaborator_duties: agendaData.assignments?.reduce((acc, a) => {
                            acc[a.collaborator] = a.duties || '';
                            return acc;
                        }, {}) || {},
                        send_whatsapp: true,
                        attachment: null
    ...
    const handleCollabToggle = (collabId) => {
        setFormData(prev => {
            const isSelected = prev.collaborators.includes(collabId);
            const newCollabs = isSelected ? prev.collaborators.filter(id => id !== collabId) : [...prev.collaborators, collabId];
            const newDuties = { ...prev.collaborator_duties };
            if (isSelected) delete newDuties[collabId];
            else newDuties[collabId] = '';
            return { ...prev, collaborators: newCollabs, collaborator_duties: newDuties };
        });
    };

    const handleDutyChange = (collabId, value) => {
        setFormData(prev => ({
            ...prev,
            collaborator_duties: { ...prev.collaborator_duties, [collabId]: value }
        }));
    };
    ...
    const handleQuickAdd = async () => {
        ...
        try {
            ...
            setFormData(prev => ({ ...prev, collaborators: [...prev.collaborators, newCollab.id] }));
            setIsQuickAddOpen(false);
            setQuickAddData({ name: '', email: '', whatsapp_number: '' });
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => {
            ...
            } else if (key === 'collaborator_duties') {
                data.append('collaborator_duties', JSON.stringify(formData[key]));
            } else if (key === 'project') {
                if (formData[key]) data.append('project_id', formData[key]);
        ...
        try {
            if (isEdit) {
                await apiClient.put(`agendas/${id}/`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
            } else {
                await apiClient.post('agendas/', data, { headers: { 'Content-Type': 'multipart/form-data' } });
            }
            navigate(-1);
        } catch (err) {
    ...
    const handleRejectSubmit = async () => {
        if (!rejectModal.reason.trim()) { toast.error('Please provide a reason'); return; }
        setResponding(true);
        try {
            await apiClient.post(`agendas/${id}/reject/`, { rejection_reason: rejectModal.reason });
    ...
    return (
        <div className="flex justify-center p-4 min-h-full animate-fade-in custom-scrollbar">
                ...
                {isEdit && myAssignment?.status === 'pending' && (
                    <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-3 text-white">
                            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center backdrop-blur-sm"><i className="fas fa-envelope-open-text"></i></div>
                            <div>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                            <button onClick={handleAccept} disabled={responding} className="flex-1 sm:flex-none px-6 py-2 bg-white text-indigo-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-50 transition-all disabled:opacity-50">
                                {responding ? <i className="fas fa-spinner fa-spin"></i> : 'Accept'}
                            </button>
                            <button onClick={() => setRejectModal({ show: true, reason: '' })} disabled={responding} className="flex-1 sm:flex-none px-6 py-2 bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-400 transition-all border border-indigo-400 disabled:opacity-50">Reject</button>
                        </div>
                    </div>
                )}
                <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 tracking-tight">{isReadOnly ? 'Task Details' : (isEdit ? (isMeeting ? 'Edit Meeting' : 'Edit Task') : (isMeeting ? 'Schedule Meeting' : 'New Task'))}</h2>
                        {formData.project && (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 space-y-4">
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    {!isReadOnly && <button type="button" onClick={() => setIsQuickAddOpen(!isQuickAddOpen)} className="text-xs text-emerald-600 hover:text-emerald-700 font-bold flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-lg hover:bg-emerald-100 transition-colors"><i className="fas fa-plus"></i> Quick Add</button>}
                                </div>
                                {isQuickAddOpen && (
                                    <div className="mb-4 p-4 bg-white rounded-xl border border-emerald-100 shadow-sm space-y-3 relative z-10 animate-fade-in"><div className="absolute -top-1.5 right-6 w-3 h-3 bg-white border-t border-l border-emerald-100 transform rotate-45"></div>                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <input type="text" placeholder="Name *" required value={quickAddData.name} onChange={(e) => setQuickAddData({ ...quickAddData, name: e.target.value })} className="text-sm px-4 py-2 border rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none" />
                                        <input type="email" placeholder="Email (for notifications)" value={quickAddData.email} onChange={(e) => setQuickAddData({ ...quickAddData, email: e.target.value })} className="text-sm px-4 py-2 border rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none" />
                                        <input type="text" placeholder="WhatsApp" value={quickAddData.whatsapp_number} onChange={(e) => setQuickAddData({ ...quickAddData, whatsapp_number: e.target.value })} className="text-sm px-4 py-2 border rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none sm:col-span-2" />
                                    </div>
                                        <div className="flex justify-end gap-2"><button type="button" onClick={() => setIsQuickAddOpen(false)} className="text-xs font-bold px-3 py-1.5 text-gray-500 hover:text-gray-700">Cancel</button><button type="button" onClick={handleQuickAdd} className="text-xs font-bold px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg shadow-sm transition-colors">Add & Select</button></div></div>
                                )}
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {formData.collaborators.map(id => {
                                        const collab = collaborators.find(c => c.id === id);
                                        return collab ? (
                                            <div key={id} className="w-full space-y-2 p-3 bg-emerald-50/50 rounded-2xl border border-emerald-100/50">
                                                <div className="flex items-center justify-between"><span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-700 text-sm font-bold rounded-full"><i className="fas fa-user text-[10px]"></i>{collab.name}</span>{!isReadOnly && <button type="button" onClick={() => handleCollabToggle(id)} className="text-gray-400 hover:text-red-500 transition-colors"><i className="fas fa-times-circle"></i></button>}</div>
                                                <div className="relative"><input type="text" disabled={isReadOnly} placeholder={`What should ${collab.name.split(' ')[0]} do?`} value={formData.collaborator_duties[id] || ''} onChange={(e) => handleDutyChange(id, e.target.value)} className="w-full pl-9 pr-4 py-2 bg-white border border-emerald-100 rounded-xl text-xs font-medium focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all disabled:bg-transparent disabled:border-transparent disabled:px-0 placeholder:italic placeholder:text-gray-300" /><div className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-300"><i className="fas fa-tasks text-[10px]"></i></div></div>
                                            </div>
                                        ) : null;
                                    })}
                                </div>
                                {!isReadOnly && (
                                    <button type="button" onClick={() => setIsCollabDropdownOpen(!isCollabDropdownOpen)} className="w-full px-5 py-3 border border-gray-200 rounded-xl bg-white text-left flex justify-between items-center hover:border-emerald-300 transition-colors"><span className={`font-medium ${formData.collaborators.length > 0 ? 'text-gray-700' : 'text-gray-400'}`}>{formData.collaborators.length > 0 ? 'Add more...' : 'Select collaborators...'}</span><i className={`fas fa-chevron-down text-xs text-gray-400 transition-transform ${isCollabDropdownOpen ? 'rotate-180' : ''}`}></i></button>
                                )}
                                {isCollabDropdownOpen && (
                                    <div className="absolute z-20 w-full mt-2 bg-white border border-gray-100 rounded-xl shadow-xl max-h-60 overflow-y-auto custom-scrollbar p-1"><div className="space-y-0.5">
                                        {collaborators.map(collaborator => (
                                            <label key={collaborator.id} className="flex items-center px-4 py-2.5 hover:bg-emerald-50 cursor-pointer rounded-lg transition-colors group"><input type="checkbox" checked={formData.collaborators.includes(collaborator.id)} onChange={() => handleCollabToggle(collaborator.id)} className="w-5 h-5 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500" /><div className="ml-3 flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-xs font-bold">{collaborator.name[0]}</div><span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">{collaborator.name}</span></div></label>
                                        ))}
                                    </div></div>
                                )}
                            </div>
                        ...
                        {isMeeting && (
                            <div className="p-5 bg-gradient-to-br from-emerald-50/50 to-indigo-50/30 rounded-[2rem] border border-emerald-100 shadow-sm relative overflow-hidden">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4 relative z-10"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-2xl bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-200 shrink-0"><i className="fas fa-video"></i></div><div className="flex-grow"><label className="block text-sm font-bold text-gray-800">Connection Link</label><p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Google Meet Integration</p></div></div>{!isReadOnly && <a href="https://meet.google.com/" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto px-4 py-2 bg-white text-emerald-600 border border-emerald-100 rounded-xl text-xs font-bold hover:bg-emerald-50 transition-all flex items-center justify-center gap-2 shadow-sm"><i className="fas fa-external-link-alt"></i>Generate Link</a>}</div>
                                <div className="flex gap-2 relative z-10"><div className="relative flex-grow"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><i className="fas fa-link text-emerald-400"></i></div><input type="text" disabled={isReadOnly} placeholder="Paste Meet link" value={formData.meeting_link} onChange={(e) => setFormData({ ...formData, meeting_link: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white border border-gray-100 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all text-sm font-medium disabled:bg-gray-50" /></div>{formData.meeting_link && <a href={formData.meeting_link} target="_blank" rel="noopener noreferrer" className="px-5 py-3 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-700 transition-all flex items-center justify-center shadow-lg shadow-emerald-100"><i className="fas fa-external-link-alt text-sm"></i></a>}</div>
                            </div>
                        )}
                        <div className="pt-5 flex gap-4 border-t border-gray-100">
                            <button type="button" onClick={() => navigate(-1)} className="flex-1 px-6 py-3.5 border border-gray-200 text-gray-600 font-bold rounded-xl text-center hover:bg-gray-50 transition-colors">{isReadOnly ? 'Close' : 'Cancel'}</button>
                            {!isReadOnly && (
                                <button type="submit" disabled={!!validationError || submitting} className={`flex-1 px-6 py-3.5 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-bold rounded-xl shadow-lg transform transition-all hover:-translate-y-0.5 ${(!!validationError || submitting) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                    {submitting ? <span className="flex items-center justify-center gap-2"><i className="fas fa-spinner fa-spin"></i><span>Saving...</span></span> : (isEdit ? 'Save Changes' : (isMeeting ? 'Schedule Meeting' : 'Create Task'))}
                                </button>
                            )}
                        </div>
                    </form>
            ...
            <ConfirmModal isOpen={rejectModal.show} onClose={() => setRejectModal({ show: false, reason: '' })} onConfirm={handleRejectSubmit} title="Reject Task" message="Are you sure you want to reject this task?" type="danger" confirmLabel="Reject">
                <div className="mt-4"><label className="block text-xs font-bold text-gray-400 uppercase mb-2">Rejection Reason *</label><textarea value={rejectModal.reason} onChange={(e) => setRejectModal({ ...rejectModal, reason: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl outline-none text-sm min-h-[100px]" placeholder="Why are you rejecting?" /></div>
            </ConfirmModal>
            <style>{`.custom-scrollbar::-webkit-scrollbar { width: 6px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; } @keyframes fade-in { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } } .animate-fade-in { animation: fade-in 0.4s ease-out; }`}</style>
        </div>
    );
};
